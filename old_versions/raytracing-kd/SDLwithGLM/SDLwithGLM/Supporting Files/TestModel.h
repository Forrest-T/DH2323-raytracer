...
public:
    ...
    {
        float x_mid =(v0.x+v1.x+v2.x)/3.f;
        float y_mid =(v0.y+v1.y+v2.y)/3.f;
        float z_mid =(v0.z+v1.z+v2.z)/3.f;
        midpoint.x = x_mid;
        midpoint.y = y_mid;
        midpoint.z = z_mid;
        //glm::vec3 mid(x_mid,y_mid,z_mid);
        //midpoint = mid;
        //glm::vec3 mid = new glm::vec3((v0+v1+v2)/3.f);
    ...
    void triangleBBox(){ // returns bounding box for a triangle
        glm::vec3 minb(std::numeric_limits<float>::max());
        glm::vec3 maxb(std::numeric_limits<float>::min());
       
        /// v0
        minb.x = std::min( minb.x, v0.x);
        minb.y = std::min( minb.y, v0.y);
        minb.z = std::min( minb.z, v0.z);
        
        maxb.x = std::max( maxb.x, v0.x);
        maxb.y = std::max( maxb.y, v0.y);
        maxb.z = std::max( maxb.z, v0.z);
